...
pdf('output/volcano_DE_radboud.pdf', width = 15, height = 5, onefile = F)
...
ggarrange(
  ggplot() +
    ...
    labs(x = 'Log-fold change ICU / healthy', y = expression(paste(-log[10], '(adj. p-value)')), color = '', title = 'ICU vs healthy') +
  ...
  ggplot() +
    ...
    labs(x = 'Log-fold change nonICU / healthy', y = expression(paste(-log[10], '(adj. p-value)')), color = '', title = 'non-ICU vs healthy') +
    scale_color_manual(values = c('#0072B5FF', '#BC3C29FF')) +
    geom_hline(yintercept = -log10(0.05), lty = 'dotted') +
    geom_vline(xintercept = 0.0, lty = 'dotted') +
    xlim(c(-5, 6)) +
  ...
  ggplot() +
    ...
    labs(x = 'Log-fold change postcovid / healthy', y = expression(paste(-log[10], '(adj. p-value)')), color = '', title = 'Postcovid vs healthy') +
    scale_color_manual(values = c('#0072B5FF', '#BC3C29FF')) +
    geom_hline(yintercept = -log10(0.05), lty = 'dotted') +
    geom_vline(xintercept = 0.0, lty = 'dotted') +
    xlim(c(-5, 6)) +
...
pdf('output/venn_biomarkers_up.pdf', width = 3, height = 3)
ggVennDiagram(x = res.up, label = 'count', label_alpha = 0) +
...
pdf('output/venn_biomarkers_down.pdf', width = 3, height = 3)
ggVennDiagram(x = res.down, label = 'count', label_alpha = 0) +
